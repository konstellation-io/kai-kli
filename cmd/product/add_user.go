...
// NewAddUserCmd creates a new command to add an user to a product.
func NewAddUserCmd(logger logging.Interface) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "add-user <product_name> <user_email> [opts...]",
		Args:  cobra.ExactArgs(2), //nolint:gomnd
		Short: "Add a user to a product",
		Annotations: map[string]string{
			"authenticated": "true",
		},
		Example: `
    $ kli product add-user <product_name> <user_email> [opts...]
		`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			err = product.NewHandler(logger, r, kaiClient.ProductClient(), kaiClient.VersionClient(), productConfigService).
				AddUser(server, productID, userEmail)
			if err != nil {
				return err
			}
