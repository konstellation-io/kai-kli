...
type ServerLoginCliSuite struct {
	suite.Suite
...
func TestServerLoginCliSuite(t *testing.T) {
	suite.Run(t, new(ServerLoginCliSuite))
}

func (s *ServerLoginCliSuite) SetupSuite() {
...
func (s *ServerLoginCliSuite) TearDownSuite(_, _ string) {
	err := os.RemoveAll(s.tmpDir)
	s.Require().NoError(err)
}

func (s *ServerLoginCliSuite) AfterTest(_, _ string) {
...
func (s *ServerLoginCliSuite) TestLoginServer_ExpectToken() {
	// Given
	...
	// WHEN
	token, err := s.manager.LoginCLI(srv.Name, srv.Realm, srv.ClientID, srv.ClientSecret, srv.Username, srv.Password)
...
func (s *ServerLoginCliSuite) TestLoginCliServer_ExpectError() {
	// Given
	err := os.MkdirAll(path.Dir(viper.GetString(config.KaiConfigPath)), 0750)
	s.Require().NoError(err)
