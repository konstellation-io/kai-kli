...
func NewAuthentication(logger logging.Interface) *Authentication {
...
func (a *Authentication) GetToken(serveName string) (*configuration.Token, error) {
...
func (a *Authentication) Login(serverName, realm, clientID, username, password string) (*configuration.Token, error) {
	kaiConfig, err := a.configService.GetConfiguration()
	if err != nil {
		return nil, err
	}
...
func (a *Authentication) loginRequest(server *configuration.Server) (*TokenResponse, error) {
...
func (a *Authentication) areCredentialsValid(server *configuration.Server) bool {
	fmt.Printf("server: %+v\n", server)
