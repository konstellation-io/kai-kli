...
func (m *MockProcessRegistryInterface) List(server *configuration.Server, productID, processType string) ([]registry.ProcessRegistry, error) {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "List", server, productID, processType)
	ret0, _ := ret[0].([]registry.ProcessRegistry)
	ret1, _ := ret[1].(error)
	return ret0, ret1
}
